...
def generate_reverse_task_data_multi_char(
    ...
    for _ in range(num_examples):
        ...
        reverse_tokens = tokens[::-1]

        d = {
            "repeat": seq_length,
            "src_seq": " ".join(tokens),
            "tgt_seq": " ".join(reverse_tokens),
            "cat": seq_length,
        }
        samples.append(d)
...
def generate_reverse_task_data_multi_char_2x(
    seq_length: int, num_examples: int
) -> List[Dict[str, str]]:
    samples: Deque[Dict[str, str]] = deque()
    ...
    for _ in range(num_examples):
        ...
        d = {
            "repeat": seq_length,
            "src_seq": " ".join(tokens),
            "tgt_seq": " ".join(reverse_tokens) + " [NEXT] " + " ".join(reverse_tokens),
            "cat": seq_length,
        }
        samples.append(d)
...
def generate_reverse_task_data_multi_char_rev_cop(
    seq_length: int, num_examples: int
) -> List[Dict[str, str]]:
    samples: Deque[Dict[str, str]] = deque()
    ...
    for _ in range(num_examples):
        ...
        d = {
            "repeat": seq_length,
            "src_seq": " ".join(tokens),
            "tgt_seq": " ".join(reverse_tokens) + " [NEXT] " + " ".join(tokens),
            "cat": seq_length,
        }
        samples.append(d)
...
def generate_reverse_task_data(
    ...
    if args.task_type == "multi_char":
        return generate_reverse_task_data_multi_char(seq_length, num_examples)
    elif args.task_type == "multi_char_2x":
        return generate_reverse_task_data_multi_char_2x(seq_length, num_examples)
    elif args.task_type == "multi_char_rev_cop":
        return generate_reverse_task_data_multi_char_rev_cop(seq_length, num_examples)
    else:
        raise ValueError(f"Invalid reverse task type: {args.task_type}")
...
def main(args: argparse.Namespace):
    ...
    for length in length_bins:
        ...
        data = generate_reverse_task_data(
            seq_length=length,
    ...
    for length in length_bins:
        num_examples = num_examples_per_bin
        bin_data = generate_reverse_task_data(
            seq_length=length,
            num_examples=int(num_examples),
            args=args,
        )
    ...
    task_prefix = {
        "multi_char": "mc",
        "multi_char_2x": "mc2x",
        "multi_char_rev_cop": "mcrv",
    }

    if args.split_name is None:
        args.split_name = (
            f"{task_prefix[args.task_type]}_tr{args.train_max}_ts{max_length}"
        )

    output_dir = Path(f"reverse_{args.split_name}") / "s2s_reverse" / args.split_name
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate Seq2seq version of Reverse dataset"
    )
