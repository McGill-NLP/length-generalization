...
def main(
    ...
    pe: str = None,
    tgt_scratchpad_config_filename: str = None,
    ...
    # results = load_dataframe_from_jsonlines(results_path)
    # results = results[results["job_type"] != "agent"]

    attn_results = load_dataframe_from_jsonlines(
        Path("results/attention_analysis.jsonl")
    )
    ...
    if launcher is None:
        launcher = "upload_experiment_with_manual_hp_attn_anlyz.sh"
    ...
    if tgt_scratchpad_config_filename is not None:
        tgt_scratchpad_config_filename = tgt_scratchpad_config_filename.split(",")
    ...
    for ds, ds_split in ds_list:
        ...
        for pe in pos_encoding_list:
            for scratchpad_config in generate_all_scratchpad_configs(ds, ds_split):
                if tgt_scratchpad_config_filename is not None:
                    if scratchpad_config["include_scratchpad"]:
                        ...
                        if (
                            ...
                            not in tgt_scratchpad_config_filename
                        ):
                            continue
                    else:
                        if "no_scratchpad" not in tgt_scratchpad_config_filename:
                            continue
                ...
                cmd += f" --tags attention_aggr_analysis,attention_aggr_analysis_{ds}"
                cmd += f" --analyze_cfg configs/attn_aggr_analysis.jsonnet"
                cmd += f" --post_script scripts/manual_sweep_launch_best_run_analyze_attn_aggr.sh"
