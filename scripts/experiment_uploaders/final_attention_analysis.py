...
def main(
    ...
    results_path = Path("results/scratch_for_attn_classic_for_attn_sanityCh_for_attn.jsonl")
    results = pd.concat(
        [
    ...
    results = results[results["job_type"] == "best_run_seed_exp"]
    ...
    for ds, ds_split in ds_list:
        ...
        for pe in pos_encoding_list:
            for _ in range(1):
                ...
                cmd += f" --tags attention_analysis,{ds_to_tag[ds]}"
