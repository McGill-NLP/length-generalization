...
def main(
    ...
    attn_results = load_dataframe_from_jsonlines(Path("results/attention_analysis.jsonl"))
    attn_results = attn_results[attn_results["job_type"] != "agent"]
    attn_results["scratchpad_config"] = attn_results["run_group"].apply(get_scratchpad_config)
    ...
    for ds, ds_split in ds_list:
        ...
        for pe in pos_encoding_list:
            ...
            for scratchpad_config in generate_all_scratchpad_configs(ds, ds_split):
                ...
                cmd += f" --tags attention_kl_analysis,attention_kl_analysis_{ds}"
