...
BASE_EXP_NAME=$(python scripts/upload_experiment.py --configs "$FINAL_CONFIGS_STR" -c "$COMMANDS" -d $DS_ID --output_only_name --use_torch_distributed)
...
python scripts/upload_experiment.py \
  ...
  --post_script ./scripts/launch_best_run.sh  \
  --group $GROUP_NAME \
