...
echo "No PE run ids: $NO_PE_RUN_IDS"
echo "------------------"
...
python scripts/upload_experiment.py \
  --configs $FINAL_CONFIGS_STR,configs/attn_kl_analysis.jsonnet \
  -c "$COMMANDS" \
  -d $DS_ID \
  --env "$ENV_STR" \
  --sweep_configs "$SWEEP_CONFIGS" \
  --sweep_name "$GROUP_NAME" \
  --post_script "scripts/manual_sweep_launch_best_run_analyze_attn_kl.sh" \
  --group $GROUP_NAME \
  --tags "$TAGS_STR" \
  --no_pe_run_ids "$NO_PE_RUN_IDS"